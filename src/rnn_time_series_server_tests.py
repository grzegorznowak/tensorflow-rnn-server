...
class RNNTimeSeriesServerTestCore(unittest.TestCase):
    ...
    def test_model_importing(self):
        stubResponse = rnn_time_series_server.load_module_method_from_path(os.path.dirname(os.path.abspath(__file__))+'/stub_module.py', 'stub_module', 'justAStubFunctionForATest')
        self.assertEqual(stubResponse(), 1234)

    def test_observation_parsing(self):
        result = rnn_time_series_server.raw_observation_to_list("10,-10,0,100,9999,0.9999,1")
